		...
		(async () => {
			try {
				...
				for (let i of Object.values(images)) {
					carouselArr.push({ img: `${import.meta.env.VITE_API_URL}/users/upload/${i}` });
				}
						...
						<>
							{
								data?.registration_status === "1" && (
									<section className='z-50 w-full md:w-[70%] flex flex-col items-center lg:flex-row lg:items-stretch gap-2 p-4'>
										<div className="w-full lg:w-[40%] mx-auto">
											<div className="sticky top-15 mx-auto">
												<Carousel
													baseWidth={320}
													autoplay={false}
													autoplayDelay={3000}
...

							{
								data?.registration_status !== "1" && (
									<div className='w-full h-[50vh] flex flex-col items-center justify-center gap-2'>
										<h1 className='text-xl font-bold'>Your profile is incomplete</h1>
										<p className='text-sm text-gray-500 mt-2'>Please complete your profile to view it here</p>
										<button
											onClick={() => navigate("/edit-profile")}
											className='bg-[#2B6969] text-white py-2 px-4 rounded-full hover:bg-[#1a8181] cursor-pointer'>
											Complete Profile
										</button>
									</div>
								)
